...
# Required variables:
# 
    DataFile     /home/caromk/simdata/base2CC_rand399c.bin       # Input data to decompose (floats multiplexed
                           #   by channel (i.e., chan1, chan2, ...))
    chans        399        # Number of data channels (= data columns) 
...
#  WeightsInFile [] # Starting ICA weight matrix (nchans,ncomps)
    ...
    lrate        2.4e-3    # Initial ICA learning rate (float << 1)
                           #  {default: heuristic ~5e-4}
    blocksize    115         # ICA block size (integer << datalength) 
    ...
    posact       off        # Make each component activation net-positive
                           # (on/off) {default: on}
    annealstep   0.90      # Annealing factor (range (0,1]) - controls 
...
#   savestep     0       # flag to turn on the weights saving mode {default:0}
# 
# Optional outputs:
# 
